...
const ROLES_F = ["All","manager","employee"];
...
const POLICY = {
  vacationAccrualStartMo: 6,
  otherEligibleDays:      90,
  vacationAccrualCapDays: 15,
...
const monthsSince = s => { const d=new Date(s),n=new Date(); return (n.getFullYear()-d.getFullYear())*12+(n.getMonth()-d.getMonth()); };
const vacEligible = e => monthsSince(e.startDate) >= POLICY.vacationAccrualStartMo;
const othEligible = e => daysSince(e.startDate)   >= POLICY.otherEligibleDays;
...
const accrualPaused = (empId, emp, reqs) =>
  (emp[emp.vacation ? "vacation" : "vacation"]?.total - usedDays(empId,"vacation",reqs)) >= POLICY.vacationAccrualCapDays;
...
export default function Dashboard() {
  ...
  const [nEmp, setNEmp] = useState({ name:"", role:"employee", department:"Project Management", startDate:"", accrualRate:3.33, email:"", ...mkB() });

  // ─── Data loading ───────────────────────────────────────────────────────────
  useEffect(() => {
    Promise.all([
      client.get('/employees').then(r => r.data),
      client.get('/requests').then(r => r.data),
    ]).then(([emps, reqs]) => {
      setEmployees(emps);
      setRequests(reqs);
    }).catch(console.error)
  ...
  async function handleAction(id, action) {
    try {
      const res = await client.put(`/requests/${id}/${action}`);
      setRequests(prev => prev.map(r => r.id !== id ? r : res.data));
  ...
  async function submitReq() {
    ...
    setSaving(true);
    try {
      const res = await client.post('/requests', { userId: emp.id, bucket: nReq.bucket, from: nReq.from, to: nReq.to, days, note: nReq.note });
      setRequests(p => [res.data, ...p]);
      setNReq({ employeeId:"", bucket:"vacation", from:"", to:"", note:"" });
      setModal(null);
    } catch(err) { console.error(err); }
  ...
  async function submitEmp() {
    if (!nEmp.name || !nEmp.role || !nEmp.email) return;
    setSaving(true);
    try {
      const res = await client.post('/employees', nEmp);
      setEmployees(p => [...p, res.data]);
      setNEmp({ name:"", role:"employee", department:"Project Management", startDate:"", accrualRate:3.33, email:"", ...mkB() });
      setModal(null);
    } catch(err) { console.error(err); }
    finally { setSaving(false); }
  }

  async function addBonusDays(empId, days) {
    try {
      const res = await client.post('/bonus', { userId: empId, days: +days });
      const newTotal = res.data.newTotal;
      const update = e => e.id !== empId ? e : { ...e, vacation: { ...e.vacation, total: newTotal } };
      setEmployees(prev => prev.map(update));
      setDetailEmp(prev => prev ? update(prev) : prev);
      setBonusInput("");
    } catch(err) { console.error(err); }
  }
  ...
  function handleXlsxFile(e) {
    ...
    reader.onload = evt => {
      try {
        ...
        const parsed=[], errs=[];
        rows.forEach((row,i) => {
          const name=String(row["Name"]||"").trim(), role=String(row["Role"]||"").trim(), email=String(row["Email"]||"").trim();
          if(!name||!role||!email){ errs.push(`Row ${i+2}: missing Name/Role/Email`); return; }
          parsed.push({ name, role:role.toLowerCase(), email, department:String(row["Department"]||"").trim(), startDate:String(row["Start Date"]||"").trim(), accrualRate:+row["Accrual Rate"]||3.33,
            vacation:{total:+row["Vacation Total"]||0}, sick:{total:+row["Sick Total"]||5}, personal:{total:+row["Personal Total"]||2}, floatHoliday:{total:+row["Float Holiday Total"]||1},
          });
        });
  ...
  async function confirmImport() {
    setSaving(true);
    try {
      const created = await Promise.all(importRows.map(r => client.post('/employees', r).then(res => res.data)));
      setEmployees(p => [...p, ...created]);
      setImportRows([]); setImportErr(""); setModal(null);
    } catch(err) { setImportErr("Import failed: " + (err.response?.data?.error || err.message)); }
    finally { setSaving(false); }
  }
  ...
  const processPdfFile = useCallback(async (file) => {
    ...
    reader.onload = async evt => {
      try {
        ...
        const res = await client.post('/pdf/parse', { base64Data, employeeNames: employees.map(e=>e.name) });
        setPdfResult(res.data);
      } catch { setPdfError("Could not parse the PDF. Check that the Anthropic key is configured."); }
  ...
  async function confirmPdfRequest() {
    if (!pdfResult) return;
    ...
    setSaving(true);
    try {
      const res = await client.post('/requests', { userId:emp.id, bucket:pdfResult.bucket||"vacation", from:pdfResult.from, to:pdfResult.to, days, note:pdfResult.note||"", source:"pdf" });
      setRequests(p => [res.data, ...p]);
      setPdfResult(null); setPdfError(""); setModal(null); setTab("requests"); setStatusF("pending");
    } catch(err) { setPdfError("Failed to save request: " + (err.response?.data?.error || err.message)); }
    finally { setSaving(false); }
  }
  ...
  if (dataLoading) {
    return (
      <div style={{ fontFamily:"'Poppins',sans-serif", minHeight:"100vh", background:"#f0f8fa", display:"flex", alignItems:"center", justifyContent:"center" }}>
        <div style={{ textAlign:"center" }}>
          ...
          <p style={{ color:"#009cbd", fontWeight:600 }}>Loading PTO data…</p>
        </div>
      </div>
    );
  }
    ...
    <div onDragOver={onDragOver} onDragLeave={onDragLeave} onDrop={onDrop}
      ...
      <div style={{ background:"#009cbd", padding:"0 28px", display:"flex", alignItems:"center", justifyContent:"space-between", height:62, position:"sticky", top:0, zIndex:50 }}>
        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
          <span style={{ color:"#fff", fontSize:17, fontWeight:700, letterSpacing:"-0.02em" }}>illuminate</span>
        ...
        <div style={{ display:"flex", gap:7, alignItems:"center" }}>
          ...
          <span style={{ color:"rgba(255,255,255,0.4)", fontSize:11, padding:"2px 7px", background:"rgba(255,255,255,0.1)", borderRadius:5, textTransform:"capitalize" }}>{user?.role}</span>
          ...
          {isAdmin && <button onClick={() => setModal("import")} style={{ padding:"7px 13px", background:"rgba(255,255,255,0.15)", border:"1px solid rgba(255,255,255,0.3)", color:"#fff", borderRadius:8, cursor:"pointer", fontSize:12, fontFamily:"inherit", fontWeight:500 }}>📥 Bulk Import</button>}
          {isAdmin && <button onClick={() => setModal("addEmp")} style={{ padding:"7px 13px", background:"rgba(255,255,255,0.15)", border:"1px solid rgba(255,255,255,0.3)", color:"#fff", borderRadius:8, cursor:"pointer", fontSize:12, fontFamily:"inherit", fontWeight:500 }}>+ Employee</button>}
          ...
          <button onClick={logout} style={{ padding:"7px 11px", background:"rgba(255,255,255,0.1)", border:"1px solid rgba(255,255,255,0.2)", color:"rgba(255,255,255,0.7)", borderRadius:8, cursor:"pointer", fontSize:12, fontFamily:"inherit" }}>Sign out</button>
        </div>
      </div>
      ...
      <div style={{ maxWidth:1320, margin:"0 auto", padding:"22px 20px" }}>
            ...
            <div key={c.l} style={{ background:"#fff", borderRadius:14, padding:"16px 20px", boxShadow:"0 1px 4px rgba(0,0,0,0.05)", border:"1px solid #ECEAE5" }}>
              <div style={{ display:"flex", justifyContent:"space-between" }}>
                <div>
                  ...
                  <p style={{ margin:"5px 0 0", fontSize:30, fontWeight:700, color:c.c, lineHeight:1 }}>{c.v}</p>
                </div>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:14, marginBottom:20 }}>
          {BUCKETS.map(b => {
            ...
            const ta = employees.reduce((s,e) => s+(e[b.key]?.total||0), 0);
            ...
            return (
              <div key={b.key} style={{ background:b.bg, borderRadius:14, padding:"14px 18px", border:`1px solid ${b.light}` }}>
                ...
                <p style={{ margin:"3px 0 7px", fontSize:20, fontWeight:700, color:b.color }}>{fmt(tu)}<span style={{ fontSize:12, fontWeight:400, opacity:0.6 }}>/{fmt(ta)}d</span></p>
        ...
        <div style={{ display:"flex", gap:8, marginBottom:14, alignItems:"center", flexWrap:"wrap" }}>
          ...
          {tab==="employees" ? (
            <>
              ...
              {ROLES_F.map(r=><button key={r} onClick={()=>setRoleF(r)} style={{ padding:"6px 11px", borderRadius:7, border:"1px solid", fontSize:11, fontWeight:500, cursor:"pointer", fontFamily:"inherit", background:roleF===r?"#009cbd":"#fff", color:roleF===r?"#fff":"#666", borderColor:roleF===r?"#009cbd":"#b8dde5", textTransform:"capitalize" }}>{r}</button>)}
            </>
          ) : (
                ...
                {filtEmps.map((emp,i) => (
                  <tr key={emp.id} style={{ borderBottom:i<filtEmps.length-1?"1px solid #F2F0EB":"none", cursor:"pointer" }}
                    ...
                    {BUCKETS.map(b => {
                      ...
                      const ineligible = b.key==="vacation" ? !vacEligible(emp) : !othEligible(emp);
                      return (
        ...
        {tab==="requests" && (
          <div style={{ display:"flex", flexDirection:"column", gap:9 }}>
            {filtReqs.map(req => {
              const sc=STATUS_META[req.status]||STATUS_META.pending, b=bk(req.bucket);
              return (
        ...
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.5)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:200, backdropFilter:"blur(5px)", padding:20 }}>
          ...
          {modal==="policy" && (
            <div style={{ background:"#fff", borderRadius:20, padding:28, width:520, maxHeight:"90vh", overflowY:"auto", boxShadow:"0 20px 60px rgba(0,0,0,0.2)" }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:22 }}>
                <h2 style={{ margin:0, fontSize:20 }}>📋 Illuminate PTO Policy</h2>
                <button onClick={closeModal} style={{ background:"none", border:"none", fontSize:18, cursor:"pointer", color:"#aaa" }}>✕</button>
              </div>
              {[
                { title:"Vacation Accrual", color:"#009cbd", bg:"#E8F5EE", light:"#C8E6D5", items:["Accrues at your personal rate per pay period (semi-monthly: 15th & last day of month, 24 periods/year)","Accrual begins 6 months after hire date","Accrual pauses once balance reaches 15 days — resumes when days are used","Vacation can go negative (borrowing against future accrual)","Cannot be cashed out","Bonus days may be awarded above the cap by management"] },
                { title:"Vacation Rollover", color:"#006d82", bg:"#E3EEF7", light:"#B8D4EA", items:["Up to 5 vacation days may roll over to the following year","Rolled-over days expire March 31 — use it or lose it","Unused rollover days after March 31 are forfeited (no cash out)"] },
                { title:"Sick Days",         color:"#006d82", bg:"#E3EEF7", light:"#B8D4EA", items:["5 sick days granted per year, reset to 0 on January 1","Eligible to use after 90 days of employment","Do not roll over — unused days are forfeited at year end","Cannot go negative"] },
                { title:"Personal Days",     color:"#009cbd", bg:"#F3E8F7", light:"#DDB8EB", items:["2 personal days granted per year, reset to 0 on January 1","Eligible to use after 90 days of employment","Do not roll over — unused days are forfeited at year end","Cannot go negative"] },
                { title:"Floating Holiday",  color:"#b08a00", bg:"#FAEEE8", light:"#F0C4AE", items:["1 floating holiday granted per year","Employee chooses when to use it","Must be used by December 31 or it is forfeited","Does not roll over, cannot go negative","Eligible to use after 90 days of employment"] },
                { title:"Day Counting",      color:"#333333", bg:"#f0f8fa", light:"#d8eef3", items:["All PTO requests count business days only (Monday–Friday)","Weekends within a request window are not counted","Applies to all buckets"] },
              ].map(section => (
                <div key={section.title} style={{ marginBottom:16, borderRadius:12, border:`1px solid ${section.light}`, overflow:"hidden" }}>
                  <div style={{ padding:"10px 14px", background:section.bg }}><p style={{ margin:0, fontWeight:700, fontSize:13, color:section.color }}>{section.title}</p></div>
            ...
            <div style={{ background:"#fff", borderRadius:20, padding:28, width:500, maxHeight:"90vh", overflowY:"auto", boxShadow:"0 20px 60px rgba(0,0,0,0.2)" }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:20 }}>
                <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                  ...
                  <div>
                    <h2 style={{ margin:0, fontSize:19 }}>{detailEmp.name}</h2>
                    <p style={{ margin:0, fontSize:12, color:"#888", textTransform:"capitalize" }}>{detailEmp.role} · {detailEmp.department}</p>
                  </div>
                </div>
                <button onClick={closeModal} style={{ background:"none", border:"none", fontSize:18, cursor:"pointer", color:"#aaa" }}>✕</button>
              </div>
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:9, marginBottom:14 }}>
                {[["Hire Date",detailEmp.startDate||"—"],["Accrual Rate",(detailEmp.accrualRate||3.33)+" hrs/period"],["Role",detailEmp.role||"—"]].map(([k,v])=>(
                  <div key={k} style={{ padding:"10px 12px", background:"#f5fbfc", borderRadius:9, border:"1px solid #F0EDE8" }}>
                    ...
                    <p style={{ margin:"2px 0 0", fontSize:13, fontWeight:600, color:"#333", textTransform:"capitalize" }}>{v}</p>
                  </div>
                ))}
              </div>
              {(!vacEligible(detailEmp)||!othEligible(detailEmp)) && (
                <div style={{ padding:"10px 13px", background:"#FFF8E7", borderRadius:9, border:"1px solid #FDECC8", marginBottom:14 }}>
                  {!vacEligible(detailEmp) && <p style={{ margin:0, fontSize:12, color:"#B76E00" }}>⏳ <b>Vacation accrual</b> starts {new Date(new Date(detailEmp.startDate).setMonth(new Date(detailEmp.startDate).getMonth()+6)).toLocaleDateString("en-US",{month:"long",day:"numeric",year:"numeric"})}</p>}
                  {!othEligible(detailEmp) && <p style={{ margin:!vacEligible(detailEmp)?"4px 0 0":0, fontSize:12, color:"#B76E00" }}>⏳ <b>Sick / Personal / Float</b> eligible after {new Date(new Date(detailEmp.startDate).getTime()+90*86400000).toLocaleDateString("en-US",{month:"long",day:"numeric",year:"numeric"})}</p>}
                </div>
              )}
              {isManager && (
              ...
              {BUCKETS.map(b => {
                ...
                const total = detailEmp[b.key]?.total || 0;
                ...
                const ineligible = b.key==="vacation" ? !vacEligible(detailEmp) : !othEligible(detailEmp);
          ...
          {modal==="pdf" && (
            <div style={{ background:"#fff", borderRadius:22, padding:28, width:500, maxHeight:"90vh", overflowY:"auto", boxShadow:"0 24px 80px rgba(0,0,0,0.25)" }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 }}>
                <h2 style={{ margin:0, fontSize:20 }}>📄 Import PTO from PDF</h2>
                <button onClick={closeModal} style={{ background:"none", border:"none", fontSize:18, cursor:"pointer", color:"#aaa" }}>✕</button>
              </div>
                ...
                return (
                  <div>
                        ...
                        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginTop:6 }}>
                          <div style={{ display:"flex", alignItems:"center", gap:9 }}>
                            ...
                            <div>
                              ...
                              {emp && <p style={{ margin:0, fontSize:11, color:"#888", textTransform:"capitalize" }}>{emp.role} · {emp.department}</p>}
                            </div>
                          </div>
                    ...
                    <div style={{ display:"flex", gap:9 }}>
                      ...
                      <button onClick={confirmPdfRequest} disabled={!emp||!pdfResult.from||!pdfResult.to||saving}
          ...
          {modal==="request" && (
            <div style={{ background:"#fff", borderRadius:20, padding:28, width:400, boxShadow:"0 20px 60px rgba(0,0,0,0.2)" }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 }}>
                <h2 style={{ margin:0, fontSize:20 }}>New PTO Request</h2>
                <button onClick={closeModal} style={{ background:"none", border:"none", fontSize:18, cursor:"pointer", color:"#aaa" }}>✕</button>
              </div>
              {[
              ...
              <button onClick={submitReq} disabled={saving} style={{ width:"100%", padding:"11px", background:saving?"#b8dde5":"#009cbd", border:"none", borderRadius:10, color:"#fff", fontWeight:700, fontSize:14, cursor:saving?"default":"pointer", fontFamily:"inherit" }}>{saving?"Submitting…":"Submit Request"}</button>
            </div>
          )}

          {/* ADD EMPLOYEE */}
          {modal==="addEmp" && isAdmin && (
            <div style={{ background:"#fff", borderRadius:20, padding:28, width:520, maxHeight:"90vh", overflowY:"auto", boxShadow:"0 20px 60px rgba(0,0,0,0.2)" }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 }}>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:18 }}>
                {[
                  ...
                  {l:"Work Email",  el:<input type="email" placeholder="jane@illuminate.net" value={nEmp.email}      onChange={e=>setNEmp(p=>({...p,email:e.target.value}))}      style={IS}/>},
                  {l:"Role",        el:<select value={nEmp.role} onChange={e=>setNEmp(p=>({...p,role:e.target.value}))} style={IS}><option value="employee">Employee</option><option value="manager">Manager</option><option value="admin">Admin</option></select>},
                  ...
                  {l:"Accrual Rate (hrs/period)", el:<input type="number" step="0.01" value={nEmp.accrualRate} onChange={e=>setNEmp(p=>({...p,accrualRate:+e.target.value}))} style={IS}/>},
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:9, marginBottom:20 }}>
                {BUCKETS.map(b => (
                  <div key={b.key} style={{ padding:"11px 13px", background:b.bg, borderRadius:10, border:`1px solid ${b.light}` }}>
                    ...
                    <input type="number" min="0" step="0.5" value={nEmp[b.key]?.total||0} onChange={e=>setNEmp(p=>({...p,[b.key]:{total:+e.target.value}}))} style={{ ...IS, background:"rgba(255,255,255,0.7)", border:`1px solid ${b.light}` }}/>
                  </div>
                ))}
              </div>
          ...
          {modal==="import" && isAdmin && (
            <div style={{ background:"#fff", borderRadius:20, padding:28, width:560, maxHeight:"90vh", overflowY:"auto", boxShadow:"0 20px 60px rgba(0,0,0,0.2)" }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 }}>
                <h2 style={{ margin:0, fontSize:20 }}>Bulk Import via Excel</h2>
                <button onClick={closeModal} style={{ background:"none", border:"none", fontSize:18, cursor:"pointer", color:"#aaa" }}>✕</button>
              </div>
              <div style={{ display:"flex", flexWrap:"wrap", gap:5, marginBottom:14 }}>
                {["Name","Email","Role","Department","Start Date","Accrual Rate","Vacation Total","Sick Total","Personal Total","Float Holiday Total"].map(c=>(
                    ...
                    <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}>
                      ...
                      <tbody>{importRows.map((r,i)=>(
                        <tr key={i} style={{ borderTop:"1px solid #F0EDE8" }}>
                          ...
                          <td style={{ padding:"6px 10px", color:"#666", textTransform:"capitalize" }}>{r.role}</td>
                          <td style={{ padding:"6px 10px", color:"#666" }}>{r.department}</td>
                          {BUCKETS.map(b=><td key={b.key} style={{ padding:"6px 10px", color:b.color, fontWeight:600 }}>{r[b.key]?.total}d</td>)}
                        </tr>
                      ))}</tbody>
                    </table>
                  </div>
                  <button onClick={confirmImport} disabled={saving} style={{ width:"100%", padding:"11px", background:saving?"#b8dde5":"#009cbd", border:"none", borderRadius:10, color:"#fff", fontWeight:700, fontSize:14, cursor:saving?"default":"pointer", fontFamily:"inherit" }}>{saving?"Importing…":`✓ Import ${importRows.length} Employee${importRows.length!==1?"s":""}`}</button>
                </>
              )}
            </div>
          )}
